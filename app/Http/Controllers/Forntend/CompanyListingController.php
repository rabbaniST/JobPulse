...
class CompanyListingController extends Controller
{
    public function index(Request $request)
    {
        ...
        $companies = Company::withCount('Job')->with('CompanyIndustry','CompanyLocation','CompanySize')->orderBy('id','desc');
        ...
        return view('forntend.company-listing', compact('companies','company_industries','company_locations','company_sizes','form_name','form_industry','form_location','form_size','form_founded','other_page_item'));
    }
    ...
    public function detail($id)
    {
        ...
        $company_single = Company::withCount('Job')->with('CompanyIndustry','CompanyLocation','CompanySize')->where('id',$id)->first();
        ...
        $jobs = Job::with('JobCategory','JobLocation','JobType','JobExperience','JobGender','JobSalaryRange')->where('company_id',$company_single->id)->get();

        $other_page_item = OtherPageItem::where('id',1)->first();

        return view('forntend.company', compact('company_single','company_photos','company_videos','jobs','other_page_item'));
    }
