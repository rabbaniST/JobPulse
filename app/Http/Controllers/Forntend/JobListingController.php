...
class JobListingController extends Controller
{
    public function index(Request $request)
    {
        ...
        $jobs = Job::with('Company','JobCategory','JobLocation','JobType','JobExperience','JobGender','JobSalaryRange')->orderBy('id','desc');
        ...
        return view('Forntend.job-listing', compact('jobs','job_categories','job_locations','job_types','job_experiences','job_genders','job_salary_ranges','form_title','form_category','form_location','form_type','form_experience','form_gender','form_salary_range','other_page_item'));
    }
    ...
    {        
        $job_single = Job::with('Company','JobCategory','JobLocation','JobType','JobExperience','JobGender','JobSalaryRange')->where('id',$id)->first();

        $jobs = Job::with('Company','JobCategory','JobLocation','JobType','JobExperience','JobGender','JobSalaryRange')->where('job_category_id',$job_single->job_category_id)->get();

        $other_page_item = OtherPageItem::where('id',1)->first();

        return view('Forntend.job', compact('job_single','jobs','other_page_item'));
    }
    ...
    public function send_email(Request $request)
    {
        ...
        Mail::to($request->receive_email)->send(new VerifyMail($subject,$message));
